...
# For usage information, run ./check_smart.pl -h
#
...
# May 13, 2011: Benjamin Renard - "Fork" in a Git repository : http://git.zionetrix.net
# Nov 29, 2012: Erwan 'Labynocle' Ben Souiden - new "Fork" : https://github.com/labynocle/nagios_plugins_by_labynocle
#                                               * change the SMART health check: change -A option by -a
#                                                                                and convert the return code
#
# Configuration
# =============
# Add this line to /etc/sudoers to let nagios user to run smartctl with root privileges:
#   "nagios        ALL=(root) NOPASSWD: /usr/sbin/smartctl"
# 
# Don't forget to install sudo command to !
# On Debian simply run: apt-get install sudo
...
my ($device, $interface, $number) = qw/0 0 0/;
if ($opt_d) {
	unless($opt_i){
